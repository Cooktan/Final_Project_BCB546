...
? Please verify if any section of this code is duplicated in the sections Readind Data/ Re-shaping Data. also, can you add a brief description of the purpose of this section? 
...
? Can you please describe a little further the next section? i.e., which dataframe we used, which is the purpose of the chunk and what the code does.
...
#### PCA
? can you add a brief description of these dfs? what we are getting from each
? Are we adding both to the final submission or just one? If both, please add a breif descrption of the difference between them
? Change path in first autoplot's ggsave(). Is generating the file in the working directory
...
? Is this one for 5e? If so, please add description (4 lines below) 
...
#### PCA
? are we saving this plot? if yes, need to add the ggsave(), if not, just delete the chunk
...
#### K-means with cycle
? can you please describe a little bit the purpose of this figure, and what the code does? 
...
? are we using this for the principal curve, or we just keep the one that tanner did? We should define that, because I think having both is redundant 
#### Principal curve
...
#create a dataframe with the top 2000 most variable
