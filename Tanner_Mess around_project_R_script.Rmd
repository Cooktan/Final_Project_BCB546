...
rna_norm %>% rownames_to_column("gene") %>% 
  ...
  separate(Cell, into = c('Cell Number', 'Rep'), sep = "_") -> test1

test1$`Cell Number` <- parse_number(test1$`Cell Number`) 

test1 %>%  
  left_join(., Table.S1[,c(1, 3)], by = "Cell Number") -> test_sum

test_sum %>% 
  ggplot(aes(x = `Anther size (mm)`, y = Zm00001d039133)) + 
  geom_point() +
  geom_smooth(se = F)
